...
// Type: LDChartControlPlus.ChartBarAcertividade
...
public class ChartBarAcertividade : UserControl
{
  private const int STEP_VALUE_Y = 10;
  private const int MAX_VALUE_Y = 100;
  private IContainer components;
  private ToolTip toolTip1;
  private List<ChartBarAcertividade.ItemsAssChart> m_itemsChart;
  ...
  public ChartBarAcertividade()
  {
    ...
    this.m_itemsChart = new List<ChartBarAcertividade.ItemsAssChart>();
    ...
    List<ChartBarAcertividade.ItemsAssChart> lstItemsChart = new List<ChartBarAcertividade.ItemsAssChart>();
    ...
    int[] numArray1 = new int[4]{ 5, 10, 15, 9 };
    int[] numArray2 = new int[4]{ 60, 45, 51, 14 };
    for (int index = 0; index < strArray.Length; ++index)
      lstItemsChart.Add(new ChartBarAcertividade.ItemsAssChart()
      {
        Color = Color.Blue,
    ...
    this.AddItems(lstItemsChart, true);
  }
  ...
  public void AddItems(
    List<ChartBarAcertividade.ItemsAssChart> lstItemsChart,
    bool convertAcertividade)
  {
    ...
    if (convertAcertividade)
    {
      for (int index = 0; index < lstItemsChart.Count; ++index)
      {
        ...
        if (num > 0)
        {
          ChartBarAcertividade.ItemsAssChart itemsAssChart = new ChartBarAcertividade.ItemsAssChart();
          itemsAssChart.Label = lstItemsChart[index].Label;
          itemsAssChart.Value = num;
          itemsAssChart.Color = lstItemsChart[index].Color;
          itemsAssChart.PesFeitas = lstItemsChart[index].Value;
          if (this.m_show90Value)
            itemsAssChart.PesFeitas90 = this.AcertividadeQtd90(lstItemsChart[index].ValueTotal);
          this.m_itemsChart.Add(itemsAssChart);
        }
      }
    }
    else
  ...
  protected override void OnPaint(PaintEventArgs e)
  {
    ...
    try
    {
      ...
      for (int index1 = 0; index1 < this.m_itemsChart.Count; ++index1)
      {
        ...
        float num16 = (float) this.m_itemsChart[index1].Value;
        ...
        SizeF sizeF9 = graphics.MeasureString(num16.ToString("#.##") + "%", this.m_barValueFont);
        PointF pointF8 = new PointF((float) ((double) rect1.Left + (double) num14 * (double) num15 + (double) num14 * 0.5 + (double) num14 * 0.5 - (double) sizeF9.Width * 0.5), num17 - sizeF9.Height * 1.2f);
        ...
        graphics.DrawString(num16.ToString("#.##") + "%", this.m_barValueFont, (Brush) new SolidBrush(Color.Navy), pointF8);
        if (this.m_show90Value && (double) num16 < 90.0)
        {
          int num18 = this.m_itemsChart[index1].PesFeitas90 - this.m_itemsChart[index1].PesFeitas;
          SizeF sizeF10 = graphics.MeasureString("F " + num18.ToString(), this.m_barValueFont);
          PointF point = new PointF((float) ((double) rect1.Left + (double) num14 * (double) num15 + (double) num14 * 0.5 + (double) num14 * 0.5 - (double) sizeF10.Width * 0.5), rect1.Top);
          graphics.DrawString("F " + num18.ToString(), this.m_barValueFont, (Brush) new SolidBrush(Color.CadetBlue), point);
        }
        if (this.m_itemsChart[index1].Label.Length > this.m_maxCountCharLineLabelX)
        {
          ...
          {
            SizeF sizeF11 = graphics.MeasureString(stringList[index4], this.m_barLabelFont);
            pointF8 = new PointF((float) ((double) rect1.Left + (double) num14 * (double) num15 + (double) num14 * 0.5 + (double) num14 * 0.5 - (double) sizeF11.Width * 0.5), (float) ((double) rect1.Bottom + 5.0 + (double) sizeF11.Height * (double) index4));
        ...
        else if (this.m_labelXDirection == DirectionLabel.Horizontal)
        {
          ...
          pointF8 = new PointF((float) ((double) rect1.Left + (double) num14 * (double) num15 + (double) num14 * 0.5 + (double) num14 * 0.5 - (double) sizeF12.Width * 0.5), rect1.Bottom + 5f);
          graphics.DrawString(this.m_itemsChart[index1].Label, this.m_barLabelFont, (Brush) new SolidBrush(this.m_barLabelColor), pointF8);
        }
        else
        {
          SizeF sizeF13 = graphics.MeasureString(this.m_itemsChart[index1].Label, this.m_barLabelFont);
          pointF8 = new PointF((float) ((double) rect1.Left + (double) num14 * (double) num15 + 10.0 + (double) num14 / 2.0 - (double) sizeF13.Height / 2.0), num7 + sizeF13.Height);
  ...
  private double Acertividade(int mailingTotal, int pesquisasFeitas)
  {
    try
    {
      return Math.Round(1.0 - Math.Round(Math.Sqrt(1.0 / Math.Round((double) -(mailingTotal * pesquisasFeitas) / Convert.ToDouble(pesquisasFeitas - mailingTotal), 6)), 9), 2) * 100.0;
    }
    catch (Exception ex)
    {
      throw ex;
  ...
  private int AcertividadeQtd90(int mailingTotal)
  {
    try
    {
      int num1 = 1;
      ...
      while (num2 < 90.0)
      {
        num2 = Math.Round(1.0 - Math.Round(Math.Sqrt(1.0 / Math.Round((double) -(mailingTotal * num1) / Convert.ToDouble(num1 - mailingTotal), 6)), 9), 2) * 100.0;
        if (num2 < 90.0)
          ++num1;
      }
      return num1;
